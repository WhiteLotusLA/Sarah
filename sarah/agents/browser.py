...
class BrowserAgent(BaseAgent):
    ...
    async def execute_script(self, script: str, context_name: str = "default") -> Any:
    ...
    async def fill_form(
        ...
        try:
            ...
            for selector, value in form_data.items():
                ...
                input_type = (
                    await element.evaluate("el => el.type") if tag == "input" else None
                )
    ...
    async def search_web(
        self, query: str, search_engine: str = "google", num_results: int = 5
    ) -> List[Dict[str, str]]:
    ...
    async def smart_click(
        self, description: str, context_name: str = "default"
    ) -> bool:
    ...
    async def wait_for_condition(
        self, condition: str, timeout: int = 30000, context_name: str = "default"
    ) -> bool:
    ...
    async def _handle_navigate(self, message):
        ...
        await self.send_command(
            message.from_agent, "navigation_complete", {"success": success, "url": url}
        )
    ...
    async def _handle_click(self, message):
        ...
        await self.send_command(
            message.from_agent, "click_complete", {"success": success}
        )
    ...
    async def _handle_type(self, message):
        ...
        await self.send_command(
            message.from_agent, "type_complete", {"success": success}
        )
    ...
    async def _handle_extract(self, message):
        ...
        extracted = await self.extract_data(
            selectors, context_name=context, as_list=as_list
        )

        await self.send_command(
            message.from_agent, "extract_complete", {"data": extracted}
        )
    ...
    async def _handle_screenshot(self, message):
        ...
        await self.send_command(
            message.from_agent, "screenshot_complete", {"image": image_data}
        )
    ...
    async def _handle_execute_script(self, message):
        ...
        await self.send_command(
            message.from_agent, "script_complete", {"result": result}
        )
    ...
    async def _handle_fill_form(self, message):
        ...
        await self.send_command(
            message.from_agent, "form_complete", {"success": success}
        )
    ...
    async def _handle_search_web(self, message):
        ...
        await self.send_command(
            message.from_agent, "search_results", {"results": results, "query": query}
        )

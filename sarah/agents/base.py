...
class BaseAgent(ABC):
    ...
    async def send_message(
        ...
        channel = (
            f"sarah:agent:{to_agent}:commands"
            if to_agent != "broadcast"
            else "sarah:broadcast:all"
        )
    ...
    async def send_command(
        ...
        """Send a command message to another agent (convenience method)"""
    ...
    def register_handler(self, message_type: Union[MessageType, str], handler: Callable):
        ...
        if isinstance(message_type, str):
            ...
            # MessageType enum handler
            self._handlers[message_type].append(handler)

    async def _listen_for_messages(self):
        """Listen for incoming messages"""
        pubsub = self.redis.pubsub()
        channels = [f"sarah:agent:{self.name}:commands", "sarah:broadcast:all"]
    ...
    async def _handle_query(self, message: AgentMessage):
        ...
        if message.payload.get("type") == "status":
            await self.send_message(
                ...
                {"status": "active", "name": self.name, "type": self.agent_type},
                correlation_id=message.id,
            )

    async def _handle_command(self, message: AgentMessage):
        """Default command handler - check for string-based command handlers"""
        command = message.payload.get("command") or message.payload.get("action")

...
class TaskAgent(BaseAgent):
    ...
    async def create_task(self, task: Task) -> Task:
        ...
        async with self.db_pool.acquire() as conn:
            ...
            """,
                ...
                (
                    json.dumps(task.recurrence_pattern)
        ...
        if self.memory:
            await self.memory.store(
                ...
                {"task_id": str(created_task.id), "task": created_task.to_dict()},
                "task_creation",
            )
    ...
    async def get_upcoming_tasks(self, days: int = 7) -> List[Task]:
        ...
        filters = {"status": TaskStatus.TODO.value, "due_before": due_before}
    ...
    async def parse_natural_language_task(self, text: str) -> Task:
        ...
        if self.memory:
            similar_tasks = await self.memory.recall(
                text, limit=3, memory_types=["task_creation"]
            )
    ...
    async def _create_recurring_task(self, original_task: Task) -> Optional[Task]:
        ...
        new_task.due_date = (
            original_task.due_date + interval if original_task.due_date else None
        )
    ...
    def _row_to_task(self, row: asyncpg.Record) -> Task:
        ...
        return Task(
            ...
            recurrence_pattern=(
                json.loads(row["recurrence_pattern"])
                if row.get("recurrence_pattern")
                else None
            ),
            dependencies=[str(d) for d in row.get("dependencies", [])],
            attachments=(
                json.loads(row["attachments"]) if row.get("attachments") else []
            ),
    ...
    async def _handle_create_task(self, message: AgentMessage) -> None:
        ...
        await self.send_command(
            message.from_agent, "task_created", {"task": created.to_dict()}
        )
    ...
    async def _handle_update_task(self, message: AgentMessage) -> None:
        ...
        await self.send_command(
            message.from_agent, "task_updated", {"task": updated.to_dict()}
        )
    ...
    async def _handle_delete_task(self, message: AgentMessage) -> None:
        ...
        await self.send_command(
            message.from_agent, "task_deleted", {"success": success, "task_id": task_id}
        )
    ...
    async def _handle_complete_task(self, message: AgentMessage) -> None:
        ...
        await self.send_command(
            message.from_agent, "task_completed", {"task": completed.to_dict()}
        )

    async def _handle_create_project(self, message: AgentMessage) -> None:
        """Handle create_project command"""
        project_data = message.payload.get("project", {})
        ...
        await self.send_command(
            ...
            "project_created",
            {
                "project": {
                    ...
                    "due_date": (
                        created.due_date.isoformat() if created.due_date else None
                    ),
                    "tags": created.tags,
    ...
    async def _handle_parse_task(self, message: AgentMessage) -> None:
        ...
        await self.send_command(
            message.from_agent, "task_parsed", {"task": task.to_dict()}
        )

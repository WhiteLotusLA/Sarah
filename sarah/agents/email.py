...
class EmailAgent(BaseAgent):
    ...
    async def reply_to_email(
        ...
        try:
            ...
            if reply_all:
                ...
                success = await self.graph_client.reply_to_message(email_id, reply_data)
    ...
    async def prioritize_email(self, email: EmailMessage) -> EmailPriority:
        ...
        # Rule-based priority
        if any(
            word in email.subject.lower() for word in ["urgent", "asap", "emergency"]
        ):
    ...
    async def detect_spam(self, email: EmailMessage) -> float:
        ...
        if email.from_address:
            ...
            if any(
                x in email.from_address for x in ["noreply", "donotreply", ".tk", ".ml"]
            ):
                spam_indicators += 1
    ...
    async def generate_auto_response(self, email: EmailMessage) -> Optional[str]:
        ...
        if (
            email.category in [EmailCategory.NEWSLETTER, EmailCategory.PROMOTIONAL]
            or email.priority == EmailPriority.SPAM
        ):
            return None
    ...
    async def _parse_email(self, msg_data: Dict[str, Any]) -> EmailMessage:
        ...
        email = EmailMessage(
            ...
            to_addresses=[
                r["emailAddress"]["address"] for r in msg_data.get("toRecipients", [])
            ],
            cc_addresses=[
                r["emailAddress"]["address"] for r in msg_data.get("ccRecipients", [])
            ],
    ...
    async def _handle_get_emails(self, message):
        ...
        await self.send_command(
            message.from_agent,
            "emails_response",
            {"emails": [self._email_to_dict(e) for e in emails], "count": len(emails)},
        )
    ...
    async def _handle_send_email(self, message):
        ...
        await self.send_command(message.from_agent, "email_sent", {"success": success})
    ...
    async def _handle_reply_email(self, message):
        ...
        await self.send_command(
            message.from_agent, "email_replied", {"success": success}
        )
    ...
    async def _handle_categorize_inbox(self, message):
        ...
        await self.send_command(
            message.from_agent, "inbox_categorized", {"count": categorized}
        )

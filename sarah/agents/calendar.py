...
class CalendarEvent:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "body": {"contentType": "text", "content": self.body or ""},
            "attendees": [
                {"emailAddress": {"address": email}} for email in self.attendees
            ],
...
class CalendarAgent(BaseAgent):
    ...
    async def update_event(
        self, event_id: str, updates: Dict[str, Any], calendar_id: Optional[str] = None
    ) -> CalendarEvent:
    ...
    async def search_events(
        self, query: str, days_back: int = 30, days_forward: int = 30
    ) -> List[CalendarEvent]:
        ...
        for event in all_events:
            if (
                query_lower in event.subject.lower()
                or (event.body and query_lower in event.body.lower())
    ...
    def _invalidate_cache(self, calendar_id: str) -> None:
        ...
        keys_to_remove = [
            k for k in self.calendar_cache.keys() if k.startswith(f"{calendar_id}:")
        ]
    ...
    async def _handle_create_event(self, message: AgentMessage) -> None:
        ...
        # Send response
        await self.send_command(
            message.from_agent, "event_created", {"event": self._event_to_dict(created)}
        )
    ...
    async def _handle_update_event(self, message: AgentMessage) -> None:
        ...
        # Send response
        await self.send_command(
            message.from_agent, "event_updated", {"event": self._event_to_dict(updated)}
        )
    ...
    async def _handle_find_free_time(self, message: AgentMessage) -> None:
        ...
        duration = message.payload["duration_minutes"]
        start = datetime.fromisoformat(message.payload["search_start"])
        end = datetime.fromisoformat(message.payload["search_end"])
        attendees = message.payload.get("attendees", [])
    ...
    async def _handle_search_events(self, message: AgentMessage) -> None:
        ...
        days_back = message.payload.get("days_back", 30)
        days_forward = message.payload.get("days_forward", 30)

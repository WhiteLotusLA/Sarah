...
class Consciousness:
    ...
    async def _delegate_to_agents(self, intent: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Fallback response
        return {
            "response": f"Hello! I'm {self.name}, your digital companion. I'm here to help!"
        }

    async def _learn_from_interaction(
        self, input: str, response: Optional[Dict[str, Any]] = None
    ) -> None:
        ...
        if self.memory:
            ...
            if hasattr(self.memory, "store_interaction"):
                ...
                await self.memory.store_interaction(
                    input, response.get("response", ""), response.get("intent")
                )
            else:

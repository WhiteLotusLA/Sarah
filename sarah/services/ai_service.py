...
class OllamaService:
    ...
    def __init__(self, base_url: Optional[str] = None):
        ...
        self._retry_config = {"max_retries": 3, "base_delay": 1.0, "max_delay": 10.0}
            ...
            async with self.session.get(f"{self.base_url}/api/tags") as response:
                if response.status == 200:
                    ...
                    self._available_models = {
                        m["name"]: m for m in data.get("models", [])
                    }
                    logger.info(
    ...
    async def _generate_stream(self, params: Dict[str, Any]) -> AsyncIterator[str]:
        ...
        params["stream"] = True
        async with self.session.post(
            f"{self.base_url}/api/generate", json=params
        ) as response:
            response.raise_for_status()
            async for line in response.content:
                if line:
    ...
    async def _chat_complete(self, params: Dict[str, Any]) -> str:
        ...
        for attempt in range(self._retry_config["max_retries"]):
            try:
                ...
                logger.warning(
                    f"Chat attempt {attempt + 1} failed, retrying in {delay}s: {e}"
                )
                await asyncio.sleep(delay)
    ...
    async def _chat_stream(self, params: Dict[str, Any]) -> AsyncIterator[str]:
        ...
        params["stream"] = True
        async with self.session.post(
            f"{self.base_url}/api/chat", json=params
        ) as response:
            response.raise_for_status()
            async for line in response.content:
                if line:

...
class MicrosoftGraphClient:
    ...
    async def _ensure_token(self) -> str:
        ...
        result = await asyncio.to_thread(
            self.msal_app.acquire_token_for_client, scopes=self.scopes
        )
    ...
    async def get_messages(
        self, folder: str = "inbox", limit: int = 50, filter_query: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """Get email messages"""
        params = {
            "$top": limit,
    ...
    async def move_message(
        self, message_id: str, destination_folder: str
    ) -> Dict[str, Any]:
    ...
    async def upload_file(
        ...
        async with self.session.put(
            f"{self.base_url}{endpoint}", headers=headers, data=content
        ) as response:
